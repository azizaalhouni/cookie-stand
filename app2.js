...
var cookiesStoresEachHour = [];
var cookiesLocationEachHour = [];
var sum = 0;
...
for(var i = 0 ; i < stores.length ; i++) {
  ...
  //cookiesLocationEachHour.push(tdElFirst.textContent);
  trEl.appendChild(tdElFirst);
  for(var j = 0 ; j < hours.length ; j++){
    var tdEl1 = document.createElement('td');
    tdEl1.textContent = stores[i].cookiesEachHourArray[j];
    //cookiesStoresEachHour[j][i] = stores[i].cookiesEachHourArray[j];
    //totalOfHourlyCookies[i] = totalOfHourlyCookies[i] + cookiesStoresEachHour[j][i];
...
// for( var n = 0 ; n < cookiesLocationEachHour.length ; n++)
// {
